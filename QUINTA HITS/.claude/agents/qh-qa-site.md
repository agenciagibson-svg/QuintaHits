...
O site é Next.js (App Router) e mora em `SITE/`. As fontes de verdade são o **Supabase**
(tabela `edicoes` = agenda; `site_config` = endereço, bairro e Instagram da casa, link de
reserva, horário padrão — ambas editadas pelo painel `/admin`) e `src/config/site.ts`
(nome da casa, textos institucionais, playlist e valores de reserva quando o painel está
vazio). Nenhuma informação pode estar escrita à mão dentro de uma página.
...
data só aparece quando você força o relógio. As funções de programação são assíncronas e
leem o Supabase: no script, transpile `programacao.ts` e `edicao.ts` com o TypeScript do
projeto e troque `supabaseAdmin` por um stub que devolve as linhas atuais de `edicoes`
(lidas pela API REST do Supabase). A lógica pura de data mora em `src/lib/edicao.ts`.
